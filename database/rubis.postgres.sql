...
DROP FUNCTION IF EXISTS to_days(timestamp with time zone);
 CREATE OR REPLACE FUNCTION to_days(i TIMESTAMP with time zone) RETURNS INT AS $$
    BEGIN
      RETURN (make_date((extract(year from i) :: int), (extract(month from i) :: int), (extract(day from i) :: int)) - date '0001-01-01') + 365;
    END;
    $$ LANGUAGE plpgsql;
