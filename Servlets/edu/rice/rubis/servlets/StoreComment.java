...
public class StoreComment extends RubisHttpServlet
{
    ...
    throws IOException, ServletException
  {
    ...
    {
      printError("You must provide a 'to user' identifier!", sp);
      sp.printHTMLfooter();
      return;
    }
    else
    ...
    if ((value == null) || (value.equals("")))
    {
      printError("You must provide a 'from user' identifier!", sp);
      sp.printHTMLfooter();
      return;
    }
    else
    ...
    if ((value == null) || (value.equals("")))
    {
      printError("You must provide a rating!", sp);
      sp.printHTMLfooter();
      return;
    }
    else
    ...
    {
      printError("You must provide a comment!", sp);
      sp.printHTMLfooter();
      return;
    }
    ...
    try
    {
      ...
      {
        conn.rollback();
        printError("Error while storing the comment.", sp);
        printException(e, sp);
        sp.printHTMLfooter();
      ...
      {
        ResultSet urs;
        stmt = conn.prepareStatement("SELECT rating FROM users WHERE id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        ...
        if (urs.first())
        {
          ...
          stmt = conn.prepareStatement("UPDATE users SET rating=? WHERE id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      ...
      {
        conn.rollback();
        printError("Error while updating user's rating.", sp);
        printException(e, sp);
        sp.printHTMLfooter();
        closeConnection(stmt, conn);
        return;
