...
public class AboutMe extends RubisHttpServlet
{
  ...
  private boolean listItem(Integer userId, Connection conn, ServletPrinter sp)
  {
    ...
    try
    {
      currentStmt =
        conn.prepareStatement(
          "SELECT * FROM items WHERE items.seller=? AND items.end_date>=NOW()", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      currentStmt.setInt(1, userId.intValue());
      currentSellings = currentStmt.executeQuery();
    }
    catch (Exception e)
    {
      printError("Exception getting current sellings list.", sp);
      printException(e, sp);
    ...
    try
    {
      ...
      else
      {
        ...
        do
        {
          ...
          {
            printError("Exception getting item.", sp);
            printException(e, sp);
            closeConnection(currentStmt, conn);
            return false;
          }
    ...
    {
      printError("Exception getting current items in sell.", sp);
      printException(e, sp);
      closeConnection(currentStmt, conn);
      return false;
    }
    ...
    try
    {
      pastStmt =
        conn.prepareStatement(
          "SELECT * FROM items WHERE items.seller=? AND TO_DAYS(NOW()) - TO_DAYS(items.end_date) < 30", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      pastStmt.setInt(1, userId.intValue());
      pastSellings = pastStmt.executeQuery();
    }
    catch (Exception e)
    {
      printError("Exception getting past sellings list.", sp);
      printException(e, sp);
    ...
    try
    {
      ...
      do
      {
        ...
        {
          printError("Exception getting sold item.", sp);
          printException(e, sp);
          closeConnection(pastStmt, conn);
          return false;
        }
    ...
    {
      printError("Exception getting sold items.", sp);
      printException(e, sp);
      closeConnection(pastStmt, conn);
      return false;
    }
  ...
  private boolean listBoughtItems(Integer userId, PreparedStatement stmt, Connection conn, ServletPrinter sp)
  {
    ...
    try
    {
      stmt =
        conn.prepareStatement(
          "SELECT * FROM buy_now WHERE buy_now.buyer_id=? AND TO_DAYS(NOW()) - TO_DAYS(buy_now.date)<=30", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    ...
    {
      printError("Exception getting bought items list.", sp);
      printException(e, sp);
      ...
      do
      {
        ...
        try
        {
          ...
          try
          {
            ...
            itemStmt =
              conn.prepareStatement("SELECT * FROM items WHERE id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
          ...
          {
            printError("Failed to execute Query for item (buy now).", sp);
            printException(e, sp);
            if (itemStmt != null) 
             itemStmt.close();
            closeConnection(stmt, conn);
          ...
          {
            printError("Failed to execute Query for seller (buy now).", sp);
            printException(e, sp);
            if (sellerStmt != null) 
             sellerStmt.close();
            closeConnection(stmt, conn);
        ...
        {
          printError("Exception getting buyNow.", sp);
          printException(e, sp);
          closeConnection(stmt, conn);
          return false;
        }
    ...
    {
      printError("Exception getting bought items.", sp);
      printException(e, sp);
      closeConnection(stmt, conn);
      return false;
    }
  ...
  private boolean listWonItems(Integer userId, PreparedStatement stmt, Connection conn, ServletPrinter sp)
  {
    ...
    try
    {
      stmt =
        conn.prepareStatement(
          "SELECT item_id FROM bids, items WHERE bids.user_id=? AND bids.item_id=items.id AND TO_DAYS(NOW()) - TO_DAYS(items.end_date) < 30 GROUP BY item_id", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      stmt.setInt(1, userId.intValue());
    ...
    {
      printError("Exception getting won items list.", sp);
      printException(e, sp);
      closeConnection(stmt, conn);
      return false;
    }
    ...
    try
    {
      do
      {
        ...
        try
        {
          ...
          {
            printError("Failed to execute Query for item (won items).", sp);
            printException(e, sp);
            if (itemStmt != null) 
             itemStmt.close();
            closeConnection(stmt, conn);
          ...
          {
            printError("Failed to execute Query for seller (won items).", sp);
            printException(e, sp);
            if (sellerStmt != null) 
             sellerStmt.close();
            closeConnection(stmt, conn);
          ...
          // 	  try 
          // 	      {
          // 		  currentPriceStmt = conn.prepareStatement("SELECT MAX(bid) AS bid FROM bids WHERE item_id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    ...
    {
      printError("Exception getting won items.", sp);
      printException(e, sp);
      closeConnection(stmt, conn);
      return false;
    }
  ...
  private boolean listComment(Integer userId, PreparedStatement stmt, Connection conn, ServletPrinter sp)
  {
    ...
    try
    {
      ...
      {
        stmt =
          conn.prepareStatement("SELECT * FROM comments WHERE to_user_id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        stmt.setInt(1, userId.intValue());
      ...
      do
      {
        ...
        try
        {
          authorStmt = conn.prepareStatement("SELECT nickname FROM users WHERE id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
  ...
  private boolean listBids(Integer userId, String username, String password, PreparedStatement stmt, Connection conn, ServletPrinter sp)
  {
    ...
    try
    {
      stmt =
        conn.prepareStatement(
          "SELECT item_id, bids.max_bid FROM bids, items WHERE bids.user_id=? AND bids.item_id=items.id AND items.end_date>=NOW() GROUP BY item_id, bids.max_bid", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      stmt.setInt(1, userId.intValue());
    ...
    {
      printError("Exception getting bids list.", sp);
      printException(e, sp);
      closeConnection(stmt, conn);
      return false;
    }
    ...
    try
    {
      do
      {
        ...
        try
        {
          itemStmt = conn.prepareStatement("SELECT * FROM items WHERE id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
          itemStmt.setInt(1, itemId);
          rs = itemStmt.executeQuery();
        }
        catch (Exception e)
        {
          printError("Failed to execute Query for item the user has bid on.", sp);
          printException(e, sp);
          if (itemStmt != null)
            itemStmt.close();
          closeConnection(stmt, conn);
        ...
        try
        {
          if (!rs.first())
          {
            sp.printHTML("<h3>No items.</h3><br>");
            itemStmt.close();
            closeConnection(stmt, conn);
            return false;
          }
          ...
          {
            printError("Failed to execute Query for seller (bids).", sp);
            printException(e, sp);
            if (itemStmt != null)
            itemStmt.close();
            if (sellerStmt != null)
    ...
    {
      printError("Exception getting items the user has bid on.", sp);
      printException(e, sp);
      closeConnection(stmt, conn);
      return false;
    }
    ...
    throws IOException, ServletException
  {
    ...
    sp.printHTMLheader("RUBiS: About");
    ...
    try
    {
      stmt = conn.prepareStatement("SELECT * FROM users WHERE id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      stmt.setInt(1, userId.intValue());
      rs = stmt.executeQuery();
    }
    catch (Exception e)
