...
public class BuyNow extends RubisHttpServlet
{
    ...
    throws IOException, ServletException
  {
    ...
    try
    {
      ...
      try
      {
        stmt = conn.prepareStatement("SELECT nickname FROM users WHERE id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      ...
      {
        printError("Failed to execute Query for seller.", sp);
        printException(e, sp);
        sp.printHTMLfooter();
        closeConnection(stmt, conn);
        return;
    ...
    // 	try
    // 	{
    // 	    stmt = conn.prepareStatement("UPDATE items SET end_date=? WHERE id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
